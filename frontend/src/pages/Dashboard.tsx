              ...
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <motion.div
                  ...
                  className={[
                    ...
                    'shadow-blue-500/25',
                    isDarkTheme
                      ? 'bg-[#0d1117] border-blue-500/20'
                      : 'bg-white border-blue-200'
                  ].join(' ')}
                  style={
                    isDarkTheme
                      ? {
                          boxShadow: `0 0 0 1px rgba(148,163,184,0.08), 0 0 20px rgba(59,130,246,0.24), 0 10px 30px rgba(2,6,23,0.45)`
                        }
                      : undefined
                  }
                >
                  {/* Glow blob */}
                  <div className="pointer-events-none absolute -right-4 -top-4 h-20 w-20 rounded-full bg-gradient-to-br from-blue-600 to-cyan-500 opacity-20 blur-2xl" />
                  <div className="relative flex items-start justify-between">
                    ...
                    <span className="flex items-center gap-0.5 rounded-full px-2 py-0.5 text-xs font-semibold bg-emerald-500/15 text-emerald-400">
                      <ArrowUpRight className="h-3 w-3" />
                      {growthPercent.toFixed(1)}%
                    </span>
                  </div>
                  <div className="mt-4">
                    <p className={['text-2xl font-black tracking-tight', isDarkTheme ? 'text-white' : 'text-slate-800'].join(' ')}>
                      {formatCurrency(Number(salesAnalysis?.totalRevenue || 0))}
                    </p>
                    <p className={['mt-1 text-xs font-medium', isDarkTheme ? 'text-slate-500' : 'text-slate-500'].join(' ')}>Receita Total</p>
                  </div>
                </motion.div>
              </div>
            </div>
